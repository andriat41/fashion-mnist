use anyhow::{anyhow, ensure, Result}; // digunakan untuk mengelola dan menangani error dengan cara yang lebih ekspresif dan nyaman
use clap::Parser; // digunakan untuk mem-parsing argumen baris perintah (command-line arguments) dalam aplikasi Rust
use image::io::Reader as ImageReader; // digunakan untuk membaca gambar dari berbagai sumber.
use std::path::{Path, PathBuf}; // digunakan untuk membuat, memanipulasi, dan menampilkan jalur file dan direktori
use tensorflow::{Graph, SavedModelBundle, SessionOptions, SessionRunArgs, Tensor}; // fungsi-fungsi standard tensorflow

const MODEL_DIR: &str = "models"; // direktori dari pemodelan yang digunakan
const TEST_DATA_DIR: &str = "images"; // direktori dari gambar yang akan digunakan
const LABELS: [&str; 10] = [
    "Top", "Trouser", "Pullover", "Dress", "Coat", "Sandal", "Shirt", "Sneaker", "Bag", "Boot",
]; // label dari gambar yang akan dianalisis
...
// fungsi untuk load gambar tester
...
// struktur klasifikasi fashion
struct FashionMnistClassifier {
    graph: Graph,
    bundle: SavedModelBundle,
}

// implementasi dari struktur diatas
